...
# Feature importance
feature_importance = rf_model.estimator_.feature_importances_
features = X.columns
...
fig_experience = px.histogram(df, x='Experience', title='Years of Experience Distribution')
st.plotly_chart(fig_experience)

# Familiarity with AI
fig_familiarity = px.histogram(df, x='AI_Familiarity', title='Familiarity with AI Technologies')
st.plotly_chart(fig_familiarity)

# AI Usage
fig_ai_usage = px.histogram(df, x='AI_Usage', title='Current AI Usage in Laboratory Operations')
st.plotly_chart(fig_ai_usage)

# Barriers to AI Implementation
fig_barriers = px.histogram(df, x='Financial_Constraints', title='Financial Constraints')
st.plotly_chart(fig_barriers)

fig_barriers_ethics = px.histogram(df, x='Ethical_Concerns', title='Ethical Concerns')
st.plotly_chart(fig_barriers_ethics)

fig_barriers_staff = px.histogram(df, x='Staff_Resistance', title='Staff Resistance to Change')
st.plotly_chart(fig_barriers_staff)

fig_barriers_training = px.histogram(df, x='Lack_Training', title='Lack of Training and Education')
st.plotly_chart(fig_barriers_training)

fig_barriers_regulatory = px.histogram(df, x='Regulatory_Compliance', title='Regulatory Compliance Issues')
st.plotly_chart(fig_barriers_regulatory)

# Recommended Strategy
fig_strategy = px.histogram(df, x='Prioritized_Strategy', title='Recommended Strategy')
st.plotly_chart(fig_strategy)

# Country Distribution Pie Chart
st.subheader("Country Distribution")
country_counts = df['Country'].value_counts()
country_names = label_encoders['Country'].inverse_transform(country_counts.index)
...
contingency_table = pd.crosstab(df['AI_Familiarity'], df['Prioritized_Strategy'])
...
spearman_corr, _ = spearmanr(df['AI_Familiarity'], df['AI_Usage'])
...
for column in X.columns:
    kruskal_results[column] = kruskal(*[group[column].values for name, group in df.groupby('Prioritized_Strategy')])
st.write(pd.DataFrame(kruskal_results, index=['Statistic', 'p-value']).T)
...
for fpr, tpr, roc_auc, model_name in roc_data:
    for i in range(n_classes):
        class_label = label_encoders["Prioritized_Strategy"].inverse_transform([i])[0]
        fig_roc.add_trace(go.Scatter(x=fpr[i], y=tpr[i], mode='lines', name=f'{model_name} - Class {class_label} (AUC = {roc_auc[i]:.2f})'))
...
model_comparison = pd.DataFrame({
    'Model': ['Random Forest', 'Gradient Boosting', 'Support Vector Machine', 'Logistic Regression', 'XGBoost'],
    'Accuracy': [rf_accuracy, gb_accuracy, svm_accuracy, log_reg_accuracy, xgb_accuracy]
})
...
st.write("The ROC curves provide a visual comparison of the models' performance, and the Kruskal-Wallis test helps in understanding the distribution differences among the target classes. The VIF values help in detecting multicollinearity among the features.")